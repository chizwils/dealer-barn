...
export default function Home() {
  ...
  // Detect entering/exiting the roadmap section and toggle lock
  useEffect(() => {
  ...
  // Route wheel to the right panel while locked (scoped to right panel)
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      e.preventDefault();
      right.scrollTop += e.deltaY; // move first
      const EPS = 2;
      const atTop = right.scrollTop <= EPS;
      const atBottom = right.scrollTop + right.clientHeight >= right.scrollHeight - EPS;
      if ((atTop && e.deltaY < 0) || (atBottom && e.deltaY > 0)) {
        setLockRoadmap(false);
      }
    };

    right.addEventListener('wheel', onWheel as EventListener, { passive: false });
    return () => {
      right.removeEventListener('wheel', onWheel as EventListener);
    };
  }, [lockRoadmap]);

  // Route touch to the right panel while locked (scoped to right panel)
  useEffect(() => {
    const right = rightPanelRef.current;
    if (!right) return;
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      e.preventDefault();
      right.scrollTop += delta; // move first
      startY = currentY;
    ...
    right.addEventListener('touchstart', onTouchStart as EventListener, { passive: false });
    right.addEventListener('touchmove', onTouchMove as EventListener, { passive: false });
  ...
  // Freeze body scroll while locked (prevents page from moving)
  useEffect(() => {
    if (lockRoadmap) {
  ...
  // Track which step is active using an observer rooted to the right pane
  useEffect(() => {
    ...
    const stepObserver = new IntersectionObserver(
      ...
      { root, threshold: 0.6, rootMargin: '-10% 0px -10% 0px' }
    );
  ...
  return (
    <div className="min-h-screen bg-neutral-50 overflow-x-hidden">
      ...
      >
        <div className="max-w-6xl mx-auto flex items-center justify-between">
            ...
            <div className="hidden md:flex items-center gap-6 text-sm">
              {['FEATURES', 'ROADMAP', 'TESTIMONIALS'].map((item) => (
                <motion.button
                  ...
                  onClick={() => {
                    const targetId = item === 'FEATURES' ? 'features' : item === 'ROADMAP' ? 'roadmap' : 'testimonials';
          ...
          <div className="flex items-center gap-4">
            <motion.button className="text-sm text-gray-700 hover:text-gray-900" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              WATCH DEMO
            </motion.button>
            <motion.button
      ...
      <section ref={heroRef} className="relative px-6 pt-32 pb-20 min-h-screen flex items-center bg-white">
        ...
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center relative z-10">
          <div>
            ...
            >
              <span className="text-sm font-medium text-gray-700">✨ Limited Beta Launch - Q2 2025</span>
            </motion.div>
            ...
            >
              DealerBarn eliminates inventory chaos, automates manual processes, and scales with your business—so you can focus on selling cars, not
              managing data disasters.
            </motion.p>
            ...
            {/* Animated Stats */}
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1, duration: 0.8 }} className="grid grid-cols-3 gap-8">
              {[
          ...
          <motion.div initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4, duration: 1, ease: 'easeOut' }} className="relative">
            ...
            {/* Floating elements */}
            <motion.div className="absolute -top-4 -right-4 w-8 h-8 bg-gray-400 rounded-full" animate={{ y: [0, -10, 0], rotate: [0, 180, 360] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }} />
            <motion.div className="absolute -bottom-4 -left-4 w-6 h-6 bg-gray-300 rounded-full" animate={{ x: [0, 10, 0], rotate: [0, -180, -360] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }} />
          </motion.div>
        </div>
      </section>
      ...
      <section className="px-6 py-20 bg-white">
        <div className="max-w-4xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="text-center mb-16">
            <h2 className="text-4xl font-roc font-medium mb-6 text-gray-900">Every Day You Don&apos;t Automate, You&apos;re Losing Money</h2>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">Most dealerships lose 15-20 hours per week on manual tasks that should take minutes.</p>
          </motion.div>

          <div className="space-y-12">
            {[
              { stat: '15-20hrs', primary: 'Lost per week', secondary: 'on manual tasks' },
              { stat: '8+', primary: 'Systems & spreadsheets', secondary: 'per dealership' },
              { stat: '20-30%', primary: 'Profit opportunities', secondary: 'missed daily' },
              { stat: '99.9%', primary: 'Manual processes', secondary: 'that should be automated' },
            ].map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1, duration: 0.6 }} viewport={{ once: true }} className="relative">
                <div className="grid lg:grid-cols-12 gap-8 items-center py-8">
                  <div className="lg:col-span-4">
                    <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.1 + 0.2, duration: 0.6 }} viewport={{ once: true }} className="text-6xl lg:text-7xl font-bold text-gray-900">
                      {item.stat}
                    </motion.div>
                  </div>
                  <div className="lg:col-span-8">
                    <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 + 0.4, duration: 0.6 }} viewport={{ once: true }} className="space-y-1">
                      <div className="text-lg font-medium text-gray-900">{item.primary}</div>
                      <div className="text-lg text-gray-600">{item.secondary}</div>
                    </motion.div>
                  </div>
                </div>
                {i < 3 && <motion.div initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} transition={{ delay: i * 0.1 + 0.6, duration: 0.8 }} viewport={{ once: true }} className="h-px border-t-2 border-dotted border-gray-200 origin-left" />}
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section id="testimonials" className="px-6 py-20 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="text-center mb-16">
            <h2 className="text-4xl font-roc font-medium text-gray-900 mb-6">Join Dealerships Already Making 23% More Profit</h2>
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-start">
            <div className="lg:col-span-8 space-y-8">
              <div className="grid md:grid-cols-2 gap-8">
                {[
                  ...
                  <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1, duration: 0.6 }} viewport={{ once: true }} className="bg-white p-6 rounded-lg border border-gray-200">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.6 }} viewport={{ once: true }} className="grid grid-cols-4 gap-8 items-center py-8">
                {[
                  ...
                  <motion.div key={i} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.1 + 0.2, duration: 0.5 }} viewport={{ once: true }} className="text-center">
                    <div className="w-16 h-16 bg-gray-200 rounded-lg flex items-center justify-center text-gray-600 font-bold text-lg mx-auto mb-2">{company.logo}</div>
            ...
            <div className="lg:col-span-4 space-y-16">
              <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: 0.3, duration: 0.6 }} viewport={{ once: true }} className="text-center">
                <div className="text-6xl font-bold text-gray-900 mb-2">90%</div>
                <div className="text-gray-600">Less manual work</div>
              </motion.div>

              <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: 0.5, duration: 0.6 }} viewport={{ once: true }} className="text-center lg:text-right">
                <div className="text-6xl font-bold text-gray-900 mb-2">50%</div>
                <div className="text-gray-600">Faster processing</div>
              </motion.div>

              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.7, duration: 0.6 }} viewport={{ once: true }} className="bg-white p-6 rounded-lg border border-gray-200 text-center">
      ...
      <section className="bg-white relative" id="roadmap" ref={roadmapSectionRef}>
        <div className="max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="text-center py-20 px-6">
            <h2 className="text-4xl font-roc font-medium text-gray-900 mb-6">Your Beta Journey</h2>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">We&apos;re building the future of dealership management, and we want you to be part of the journey.</p>
          </motion.div>
          ...
          <div className="flex min-h-screen">
            ...
            <div className="w-1/3 px-6">
              <div className="sticky top-24">
                <div className="w-full space-y-2 py-8">
                  {[
                    { id: 'beta-onboarding', title: 'Beta Onboarding', date: 'Q2 2025' },
                    { id: 'automation-rollout', title: 'Automation Rollout', date: 'Q3 2025' },
                    { id: 'advanced-analytics', title: 'Advanced Analytics', date: 'Q4 2025' },
                    { id: 'scale-expand', title: 'Scale & Expand', date: 'Q1 2026' },
                  ].map((step, i) => (
                    <motion.button
                      key={step.id}
                      ...
                      onClick={() => {
                        ...
                        if (targetElement && rightPanel) {
                          ...
                          rightPanel.scrollTo({ top: elementTop, behavior: 'smooth' });
                        }
                      }}
                      className={`w-full text-left p-4 rounded-lg transition-all duration-300 border-l-4 ${
                        activeStep === i ? 'bg-gray-100 text-gray-900 border-l-gray-400' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 border-l-transparent'
                      }`}
                    >
            ...
            <div className="flex-1 px-6">
              <div
                id="roadmap-content"
                ref={rightPanelRef}
                className="space-y-12 py-8 h-[calc(100vh-8rem)] overflow-y-auto scroll-smooth snap-y snap-mandatory"
              >
                {/* Beta Onboarding */}
                <motion.div
                  id="beta-onboarding"
                    ...
                    <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-sm">
                      ...
                      <div className="space-y-4">
                        {[
                          ...
                          <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05, duration: 0.4 }} viewport={{ once: true }} className="flex items-center gap-3 py-2">
                  ...
                  <div className="w-full">
                    <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-sm">
                      <div className="mb-8">
                        ...
                        <p className="text-xl text-gray-600 leading-relaxed">Activate automated workflows and eliminate manual processes that have been slowing down your operations.</p>
                      </div>
                  ...
                  <div className="w-full">
                    <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-sm">
                      <div className="mb-8">
                        ...
                        <p className="text-xl text-gray-600 leading-relaxed">Unlock predictive insights and advanced reporting capabilities to make data-driven decisions.</p>
                      </div>
                  ...
                  <div className="w-full">
                    <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-sm">
                      <div className="mb-8">
                        ...
                        <p className="text-xl text-gray-600 leading-relaxed">Add new locations and scale your operations seamlessly across multiple dealership sites.</p>
                      </div>
          ...
          {/* Spacer to let the section hand off scroll cleanly */}
      ...
      <section className="px-6 py-20 bg-blue-800 text-white">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="grid sm:grid-cols-2 gap-6 mb-12 max-w-2xl mx-auto">
            {['First 90 days free (worth $2,400)', 'White-glove onboarding included', 'Direct line to our product team', 'Lock in founding member pricing'].map((prop, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1, duration: 0.6 }} viewport={{ once: true }} className="flex items-center gap-3 text-left">
          ...
          <motion.form initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.6 }} viewport={{ once: true }} className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto mb-6">
            <input
              type="email"
          ...
          <p className="text-blue-200 text-sm">We respect your privacy. Unsubscribe anytime.</p>
        </div>
      </section>
